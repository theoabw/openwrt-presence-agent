...
func Parse(args []string) (Config, error) {
	...
	fs.DurationVar(&c.ReconnectGrace, "reconnect-grace", c.ReconnectGrace, "keep a client present for this long after its last connection is lost, waiting for a quick reconnection (0 disables)")
...
func (c Config) Validate() error {
	...
	if c.ReconnectGrace < 0 || c.ReconnectGrace > 10*time.Minute {
		return fmt.Errorf("reconnect-grace must be between 0 and 10m")
	}
	if c.ReconnectGrace > 0 && c.ReconnectGrace < time.Second {
		return fmt.Errorf("reconnect-grace must be at least 1s when enabled")
	}
	return nil
}
