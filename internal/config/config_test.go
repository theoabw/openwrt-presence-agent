...
func TestUCIDefaultsMatchBinaryDefaults(t *testing.T) {
	...
	expected := map[string]string{
		...
		"departure_delay":          c.DepartureDelay.String(),
	}
...
func TestDepartureDelayValidationBounds(t *testing.T) {
	valid := []time.Duration{0, time.Second, 5 * time.Minute, 10 * time.Minute}
	for _, d := range valid {
		c := Default()
		c.DepartureDelay = d
		if err := c.Validate(); err != nil {
			t.Errorf("departure-delay %v rejected: %v", d, err)
		}
	}
	...
	for _, d := range invalid {
		...
		c.DepartureDelay = d
		if err := c.Validate(); err == nil {
			t.Errorf("departure-delay %v accepted", d)
		}
	}
}
